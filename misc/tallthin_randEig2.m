function out = tallthin_randEig2(in)
% out = tallthin_randEig2(in)
%
...
%  leverage score based randEig Nystrom extension to A, using l columns sampled 
%  according to approximate leverage score probabilities. The first row corresponds to 
%  randEig extensions where the rank was not fixed, the second to
%  extensions where the rank was fixed
% 
%  -timings, a two row vector of the time it took to run each experiment 
%  (i.e. form Q,W,Wk), including the time to approximate the leverage 
%  scores
%
